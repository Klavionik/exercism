...
fn check_rule_3(input: &str) -> bool {
    input
        .find("qu")
        .is_some_and(|idx| input[..idx].chars().all(|letter| is_consonant(&letter)))
}

fn check_rule_4(input: &str) -> bool {
    input
        .find("y")
        .is_some_and(|idx| idx > 0 && input[..idx].chars().all(|letter| is_consonant(&letter)))
}
