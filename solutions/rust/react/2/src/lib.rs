...
// Performs topological sort on the dependencies tree starting at the given root.
pub fn topo(nodes: &HashMap<CellId, Node>, root: &Node) -> Vec<CellId> {
...
impl<'a, T: Copy + PartialEq + Debug> Reactor<'a, T> {
    ...
    pub fn create_compute(
        ...
        for dep in dependencies {
            match dep {
                CellId::Input(id) => match self.inputs.get(id) {
                    None => return Err(*dep),
                    Some(cell) => initial_values.push(cell.value)
                },
                CellId::Compute(id) => match self.computes.get(id) {
                    None => return Err(*dep),
                    Some(cell) => initial_values.push(cell.value)
                },
            };
        ...
        let compute = ComputeCell::new(compute_func, &initial_values);
    ...
    pub fn set_value(&mut self, id: InputCellId, new_value: T) -> bool {
        ...
        self.inputs.get_mut(&id).unwrap().set_value(new_value);
        
        // Update nodes in a topological (dependency) order.
        let topo_order = topo(&self.nodes, self.nodes.get(&CellId::Input(id)).unwrap());
        ...
        for node_id in topo_order.iter() {
            ...
            for parent in &node.parents {
                match parent {
                    CellId::Input(id) => {
                        let cell = self.inputs.get(id).unwrap();
                        initial_values.push(cell.value)
                    }
                    CellId::Compute(id) => {
    ...
    pub fn add_callback<C: Fn(T) + 'a>(
        ...
        let compute = self.computes.get_mut(&id).unwrap();
